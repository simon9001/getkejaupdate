...
export class CaretakerService {
    ...
    async getDashboardStats(userId) {
        ...
        const [openMaintenance, pendingComplaints, upcomingMoveIns, upcomingMoveOuts, overdueRent, unreadNotifications,] = await Promise.all([
    ...
    async getPropertyUnits(userId, propertyId) {
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    async getTenants(userId, propertyId, unitId) {
    ...
    async getTenantDetails(userId, tenantId) {
      ...
      `)
        ...
        const hasAccess = (data.long_term_bookings ?? []).some((booking) => accessibleProperties.includes(booking.property_id));
        if (!hasAccess)
            throw new Error('Forbidden');
        return data;
    }
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    async getRentCollections(userId, propertyId, month) {
        // Verify caretaker has rent collection permission
        const canCollect = await this._verifyRentCollectionPermission(userId);
        if (!canCollect)
            throw new Error('Forbidden: You do not have permission to record rent');
        const accessibleProperties = await this._getAccessiblePropertyIds(userId);
        let query = supabaseAdmin
            .from('rent_payments')
            .select(`
      ...
      `)
        ...
        const { data, error } = await query;
        if (error)
            throw new Error(`Failed to fetch rent collections: ${error.message}`);
        return data ?? [];
    }
    async recordRentPayment(userId, payment) {
        ...
        if (!canCollect)
            throw new Error('Forbidden: You do not have permission to record rent for this property');
        const { data, error } = await supabaseAdmin
            .from('rent_payments')
            .insert({
      ...
      `)
        ...
        for (const tenancy of tenancies ?? []) {
            ...
            if (!byProperty[propId]) {
                byProperty[propId] = {
                    property_name: tenancy.property?.title ?? 'Unknown',
                    expected: 0,
                    collected: 0,
                };
            }
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    async getMaintenanceRequests(userId, status, propertyId) {
        const accessibleProperties = await this._getAccessiblePropertyIds(userId);
        let query = supabaseAdmin
            .from('maintenance_requests')
            .select(`
    ...
    async createMaintenanceRequest(userId, request) {
        // Verify caretaker has access to this property
    ...
    async updateMaintenanceStatus(userId, requestId, status, notes) {
    ...
    async completeMaintenance(userId, requestId, completion) {
        const hasAccess = await this._verifyMaintenanceRequestAccess(userId, requestId);
        if (!hasAccess)
            throw new Error('Forbidden');
        const { data, error } = await supabaseAdmin
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    async getComplaints(userId, status, propertyId) {
        const accessibleProperties = await this._getAccessiblePropertyIds(userId);
        let query = supabaseAdmin
            .from('complaints')
            .select(`
    ...
    async resolveComplaint(userId, complaintId, resolutionNotes) {
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    async getUpcomingMoveIns(userId, upcomingDays = 30) {
        const accessibleProperties = await this._getAccessiblePropertyIds(userId);
      ...
      `)
        ...
        if (error)
            throw new Error(`Failed to fetch move-ins: ${error.message}`);
        return data ?? [];
    }
    async getUpcomingMoveOuts(userId, upcomingDays = 30) {
        const accessibleProperties = await this._getAccessiblePropertyIds(userId);
        const endDate = new Date(Date.now() + upcomingDays * 86400000).toISOString().split('T')[0];
        const today = new Date().toISOString().split('T')[0];
        const { data, error } = await supabaseAdmin
      ...
      `)
        ...
        if (error)
            throw new Error(`Failed to fetch move-outs: ${error.message}`);
        return data ?? [];
    }
    async confirmMoveIn(userId, bookingId, inspection) {
        ...
        const { error: bookingError } = await supabaseAdmin
            ...
            .eq('id', bookingId);
        if (bookingError)
            throw new Error(`Failed to confirm move-in: ${bookingError.message}`);
        ...
        const { data, error } = await supabaseAdmin
            ...
            .single();
        if (error)
            throw new Error(`Failed to record move-in: ${error.message}`);
        // Add photos if provided
    ...
    async confirmMoveOut(userId, bookingId, inspection) {
        const hasAccess = await this._verifyBookingAccess(userId, bookingId);
        if (!hasAccess)
            throw new Error('Forbidden');
        ...
        const { error: bookingError } = await supabaseAdmin
            ...
            .eq('id', bookingId);
        if (bookingError)
            throw new Error(`Failed to confirm move-out: ${bookingError.message}`);
        ...
        const { data, error } = await supabaseAdmin
            ...
            .single();
        if (error)
            throw new Error(`Failed to record move-out: ${error.message}`);
        // Add photos if provided
        if (inspection.photos && inspection.photos.length > 0) {
            for (const photo of inspection.photos) {
                await supabaseAdmin
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    async getActivityLogs(userId, date, propertyId) {
        let query = supabaseAdmin
            .from('activity_logs')
            .select(`
    ...
    async createActivityLog(userId, log) {
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    async getInventory(userId, propertyId, unitId) {
        const accessibleProperties = await this._getAccessiblePropertyIds(userId);
        let query = supabaseAdmin
    ...
    async updateInventoryItem(userId, itemId, updates) {
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    async getVisitorLogs(userId, propertyId, date) {
        const accessibleProperties = await this._getAccessiblePropertyIds(userId);
        let query = supabaseAdmin
            .from('visitor_logs')
            .select(`
      ...
      `)
        ...
        const { data, error } = await query;
        if (error)
            throw new Error(`Failed to fetch visitor logs: ${error.message}`);
        return data ?? [];
    }
    async logVisitor(userId, visitor) {
        ...
        const { data, error } = await supabaseAdmin
            ...
            .single();
        if (error)
            throw new Error(`Failed to log visitor: ${error.message}`);
        logger.info({ userId, visitorName: visitor.visitor_name }, 'caretaker.visitor.logged');
        return data;
    }
    async visitorCheckout(userId, visitorId) {
        const { data, error } = await supabaseAdmin
            .from('visitor_logs')
            .update({
            ...
            .single();
        if (error)
            throw new Error(`Failed to record visitor checkout: ${error.message}`);
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    async getUtilityReadings(userId, propertyId, unitId, month) {
        const accessibleProperties = await this._getAccessiblePropertyIds(userId);
        let query = supabaseAdmin
      ...
      `)
        ...
        const { data, error } = await query;
        if (error)
            throw new Error(`Failed to fetch utility readings: ${error.message}`);
        return data ?? [];
    }
    async submitUtilityReading(userId, reading) {
        ...
        const { data, error } = await supabaseAdmin
            ...
            .single();
        if (error)
            throw new Error(`Failed to submit utility reading: ${error.message}`);
    ...
    async updateProfile(userId, updates) {
        ...
        if (updates.lives_on_compound !== undefined || updates.emergency_contact) {
            const { error } = await supabaseAdmin
                ...
                .eq('user_id', userId);
            if (error)
                throw new Error(`Failed to update caretaker profile: ${error.message}`);
        }
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    async _getCaretakerAssignments(userId) {
        const { data, error } = await supabaseAdmin
    ...
    async _getAccessiblePropertyIds(userId) {
        const assignments = await this._getCaretakerAssignments(userId);
        const propertyIds = assignments.map(a => a.property_id).filter(Boolean);
    ...
    async _verifyPropertyRentCollectionPermission(userId, propertyId) {
        const { data, error } = await supabaseAdmin
            .from('caretaker_assignments')
            .select('can_collect_rent')
            .eq('caretaker_user_id', userId)
    ...
    async _verifyMaintenanceRequestAccess(userId, requestId) {
        const { data, error } = await supabaseAdmin
            .from('maintenance_requests')
            .select('property_id')
            .eq('id', requestId)
    ...
    async _verifyComplaintAccess(userId, complaintId) {
        const { data, error } = await supabaseAdmin
            .from('complaints')
            .select('property_id')
            .eq('id', complaintId)
    ...
    async _verifyBookingAccess(userId, bookingId) {
        const { data, error } = await supabaseAdmin
            .from('long_term_bookings')
            .select('property_id')
            .eq('id', bookingId)
    ...
    async _verifyInventoryAccess(userId, itemId) {
        const { data, error } = await supabaseAdmin
            .from('inventory_items')
            .select('property_id')
            .eq('id', itemId)
    ...
    async _getRequestNotes(requestId) {
