...
app.use('*', async (c, next) => {
    ...
    try {
        ...
        metrics.httpRequestDuration.observe({ method: c.req.method, path: c.req.path }, durationSec);
        const logPayload = {
            requestId,
            method: c.req.method,
            path: c.req.path,
        ...
        if (status >= 500)
            logger.error(logPayload, 'HTTP 5xx');
        else if (status >= 400)
            logger.warn(logPayload, 'HTTP 4xx');
        else
            logger.info(logPayload, 'HTTP 2xx/3xx');
    }
});
// ─────────────────────────────────────────────────────────────────────────────
// 5. Rate limiting
...
// ─────────────────────────────────────────────────────────────────────────────
app.notFound((c) => {
    logger.warn({ requestId: c.get('requestId'), method: c.req.method, path: c.req.path }, 'route.not_found');
    return c.json({ message: 'Route not found', path: c.req.path, code: 'NOT_FOUND' }, 404);
});
// ─────────────────────────────────────────────────────────────────────────────
...
process.on('uncaughtException', (err) => {
    logger.fatal({ message: err.message, stack: err.stack, timestamp: new Date().toISOString() }, 'process.uncaughtException');
    process.exit(1);
});
/**
...
process.on('unhandledRejection', (reason) => {
    logger.error({ reason, timestamp: new Date().toISOString() }, 'process.unhandledRejection');
});
export default app;
