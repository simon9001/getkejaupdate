...
function buildProfile(user, profile, roles) {
    return {
        id: user.id,
        email: user.email,
...
export class UsersService {
    ...
    // -------------------------------------------------------------------------
    async upsertSeekerProfile(userId, body) {
        const { error } = await supabaseAdmin
            .from('seeker_profiles')
    ...
    async upsertLandlordProfile(userId, body) {
        const { error } = await supabaseAdmin
            .from('landlord_profiles')
            .upsert({ user_id: userId, ...body, updated_at: new Date().toISOString() }, { onConflict: 'user_id' });
        if (error)
    ...
    async upsertAgentProfile(userId, body) {
        const { error } = await supabaseAdmin
            .from('agent_profiles')
            .upsert({ user_id: userId, ...body, updated_at: new Date().toISOString() }, { onConflict: 'user_id' });
        if (error)
    ...
    async upsertCaretakerProfile(userId, body) {
        const { error } = await supabaseAdmin
            .from('caretaker_profiles')
            .upsert({ user_id: userId, ...body, updated_at: new Date().toISOString() }, { onConflict: 'user_id' });
        if (error)
    ...
    async upsertDeveloperProfile(userId, body) {
    ...
    async getAllUsers(page = 1, limit = 10, search = '') {
        ...
        let query = supabaseAdmin
            .from('users')
            .select(`id, email, phone_number, account_status, email_verified, auth_provider, created_at,
         user_profiles(full_name, display_name, avatar_url, county),
         user_roles(is_active, roles(name))`, { count: 'exact' })
            .is('deleted_at', null);
        if (search) {
    ...
    async updateUserStatus(userId, status) {
        const VALID = ['active', 'suspended', 'pending_verify', 'banned'];
    ...
    // -------------------------------------------------------------------------
    async updateUserRoles(userId, ops) {
        const { assign = [], revoke = [] } = ops;
        // Resolve role names → IDs
        const names = [...new Set([...assign, ...revoke])];
        if (names.length === 0)
            return { success: true, message: 'No role changes requested' };
        ...
        for (const roleName of assign) {
            ...
            await supabaseAdmin
                .from('user_roles')
                .upsert({ user_id: userId, role_id: roleId, is_active: true, assigned_at: new Date().toISOString() }, { onConflict: 'user_id,role_id' });
        }
        return { success: true };
    }
    // -------------------------------------------------------------------------
    ...
    // -------------------------------------------------------------------------
    async submitVerification(userId, body) {
