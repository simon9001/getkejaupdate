...
export class AdminService {
    ...
    async getKpiSnapshot() {
        ...
        const [totalUsers, newUsersMonth, totalProperties, newPropertiesMonth, activeListings, totalShortStayBookings, confirmedBookings, totalLtBookings, activeTenancies, totalRevenue, revenueMonth, pendingVerifications, openDisputes, fraudQueueCount, activeSubscriptions, pendingVisits,] = await Promise.all([
            // Users
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    async getRevenueBreakdown(fromDate, toDate) {
        ...
        for (const l of listing.data ?? []) {
            const tier = l.listing_fee_tiers?.tier_name ?? 'unknown';
    ...
    async getDailyRevenueSeries(days = 30) {
        ...
        if (error)
            throw new Error(`Failed to fetch revenue series: ${error.message}`);
        // Pivot: group by day, each stream as a column
        const series = {};
        for (const row of data ?? []) {
            const day = String(row.day);
            if (!series[day])
                series[day] = { listing_fee: 0, viewing_fee: 0, subscription: 0 };
            series[day][row.revenue_type] = round2(Number(row.total_kes));
        }
        return {
            days,
            since,
    ...
    async getUserStats() {
        const monthStart = startOfMonth();
        const [byStatus, byProvider, byRole, newMonthly, verifiedIds,] = await Promise.all([
        ...
        for (const u of byProvider.data ?? []) {
            const p = u.auth_provider;
            providerCounts[p] = (providerCounts[p] ?? 0) + 1;
        }
        // Aggregate by_role
        const roleCounts = {};
        for (const r of byRole.data ?? []) {
            const name = r.roles?.name ?? 'unknown';
     ...
     */
    async getUserActivityProfile(userId) {
        const [userRow, sessions, properties, guestBookings, hostBookings, ltTenantBookings, ltLandlordBookings, givenReviews, subscription, auditLog,] = await Promise.all([
            supabaseAdmin
                .from('users')
                .select(`
    ...
    async getPropertyStats() {
        const [byCategory, byStatus, byType, byFurnished, featuredCount, withBoosts, avgScores,] = await Promise.all([
    ...
    async getPropertiesNeedingAttention(page = 1, limit = 20) {
      ...
      `, { count: 'exact' })
            .is('deleted_at', null)
            .eq('status', 'available')
            .not('id', 'in', `(SELECT DISTINCT property_id FROM property_media)`)
            .order('created_at', { ascending: false })
    ...
    async getBookingStats() {
        ...
        ssAll, ssMonth, ssConfirmed, ssCancelled, ssDisputed, ssEscrowHeld, ssPayoutReleased, 
        // Long-term by status
        ltAll, ltPending, ltActive, ltTerminated, 
        // Visits
        visRequested, visCompleted, visNoShow,] = await Promise.all([
            supabaseAdmin.from('short_stay_bookings').select('id', { count: 'exact', head: true }),
     ...
     */
    async listAllShortStayBookings(page = 1, limit = 20, filters = {}) {
        const from = (page - 1) * limit;
        let q = supabaseAdmin
            .from('short_stay_bookings')
            .select(`
     ...
     */
    async listAllLongTermBookings(page = 1, limit = 20, filters = {}) {
        const from = (page - 1) * limit;
        let q = supabaseAdmin
            .from('long_term_bookings')
            .select(`
      ...
      `, { count: 'exact' })
        ...
        const { data, count, error } = await q;
        if (error)
            throw new Error(`Failed to list long-term bookings: ${error.message}`);
        return { bookings: data ?? [], total: count ?? 0, page, limit, pages: Math.ceil((count ?? 0) / limit) };
    }
    // ═══════════════════════════════════════════════════════════════════════════
     ...
     */
    async resolveMessageReport(reportId, adminId) {
    ...
    async listSubscriptionPlans() {
      ...
      `)
        ...
        for (const c of counts ?? []) {
            ...
            if (c.status === 'active')
                activeCounts[c.plan_id] = (activeCounts[c.plan_id] ?? 0) + 1;
        }
        return (plans ?? []).map((p) => ({
            ...p,
     ...
     */
    async updateSubscriptionPlan(planId, adminId, updates) {
        const { error } = await supabaseAdmin
            .from('subscription_plans')
    ...
    async updateFeeConfigEntry(configKey, newValue, adminId) {
        const { error } = await supabaseAdmin
            .from('fee_config')
    ...
    async updateViewingFee(feeId, updates, adminId) {
        const { error } = await supabaseAdmin
            .from('viewing_fee_config')
            .update({ ...updates, updated_by: adminId, updated_at: new Date().toISOString() })
            .eq('id', feeId);
        if (error)
            throw new Error(`Failed to update viewing fee: ${error.message}`);
        logger.info({ feeId, updates, adminId }, 'admin.viewing_fee.updated');
        return { success: true };
    }
    async updateBoostPackage(packageId, updates, adminId) {
        const { error } = await supabaseAdmin
            .from('boost_packages')
            .update({ ...updates, updated_by: adminId, updated_at: new Date().toISOString() })
            .eq('id', packageId);
        if (error)
            throw new Error(`Failed to update boost package: ${error.message}`);
    ...
    async getTopListings(limit = 20) {
      ...
      `)
            ...
            .limit(limit);
        if (error)
            throw new Error(`Failed to fetch top listings: ${error.message}`);
        return data ?? [];
    }
    // ═══════════════════════════════════════════════════════════════════════════
    ...
    async listAdCampaigns(page = 1, limit = 20, status) {
        const from = (page - 1) * limit;
        let q = supabaseAdmin
            .from('ad_campaigns')
            .select(`
    ...
    async approveAdCampaign(campaignId, adminId) {
        const { error } = await supabaseAdmin
            .from('ad_campaigns')
    ...
    async getReviewStats() {
        const [totalPublished, totalHeld, totalRejected, avgRating, fiveStarCount, oneStarCount, fraudSignalsByType,] = await Promise.all([
        ...
        for (const s of fraudSignalsByType.data ?? []) {
            const sig = s.signal;
            if (!signalCounts[sig])
                signalCounts[sig] = { total: 0, high: 0 };
            signalCounts[sig].total++;
            if (s.confidence === 'high')
                signalCounts[sig].high++;
        }
        return {
    ...
    async getPendingListings(page = 1, limit = 20) {
      ...
      `, { count: 'exact' })
        ...
        const listings = (data ?? []).map((p) => {
            ...
            const coverMedia = (p.property_media ?? []).find((m) => m.is_cover) ?? (p.property_media ?? [])[0];
            return {
                id: p.id,
                title: p.title,
                status: p.status,
     ...
     */
    async approveListing(propertyId, adminId) {
        const { error } = await supabaseAdmin
            .from('properties')
     ...
     */
    async rejectListing(propertyId, adminId, reason) {
        const { error } = await supabaseAdmin
            .from('properties')
    ...
    async approveVerification(verificationId, adminId) {
      ...
      `)
            ...
            .single();
        if (fetchErr || !ver)
            throw new Error(`Failed to fetch verification record: ${fetchErr?.message ?? 'not found'}`);
        ...
        const assignedRole = DOC_TO_ROLE[ver.doc_type] ?? 'landlord';
        ...
        const { error: approveErr } = await supabaseAdmin
            ...
            .eq('id', verificationId);
        if (approveErr)
            throw new Error(`Failed to approve verification: ${approveErr.message}`);
        ...
        // Send approval email
        const userData = ver.users;
        if (userData?.email) {
            await emailService.sendVerificationApprovedEmail(userData.email, userData.user_profiles?.full_name || 'Member', assignedRole).catch(err => logger.error({ err, userId: ver.user_id }, 'admin.approve.email_failed'));
        }
    ...
    async rejectVerification(verificationId, adminId, reason) {
      ...
      `)
        ...
        const userData = ver.users;
        if (userData?.email) {
            await emailService.sendVerificationRejectedEmail(userData.email, userData.user_profiles?.full_name || 'Member', reason).catch(err => logger.error({ err, userId: ver.user_id }, 'admin.reject.email_failed'));
        }
    ...
    async _getRevenueInRange(from, to) {
        const [l, v, s] = await Promise.all([
