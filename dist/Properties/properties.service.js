...
import { uploadPropertyMedia, uploadPropertyMediaBatch, deletePropertyMedia, deletePropertyMediaByTag, } from '../utils/cloudinary.js';
// =============================================================================
...
function computeDistanceResult(propLat, propLon, placeLat, placeLon) {
...
export class PropertiesService {
     ...
     */
    async createProperty(createdBy, input) {
        const { location, pricing, contacts, media = [], amenities = [], nearby_places = [], rental_unit, short_term_config, commercial_config, plot_details, offplan_details, ...coreFields } = input;
        // ── 1. Core property row ─────────────────────────────────────────────
        ...
        if (media.length > 0) {
            try {
                const uploaded = await uploadPropertyMediaBatch(media.map((m, idx) => ({
                    fileSource: m.file,
                    options: {
                ...
                const { error: mediaErr } = await supabaseAdmin
                    .from('property_media')
                    .insert(uploaded.map((result, idx) => ({
                    property_id: propertyId,
                ...
                logger.error({ err: uploadErr?.message, propertyId }, 'properties.media.cloudinary_upload.failed');
            }
        }
        // ── 6. Amenities ─────────────────────────────────────────────────────
        ...
        if (nearby_places.length > 0) {
            ...
            const placesRows = nearby_places.map((place) => {
                const { distanceM, walkMinutes, driveMinutes } = computeDistanceResult(propLat, propLon, place.latitude, place.longitude);
                return {
                    property_id: propertyId,
            ...
            if (placesErr) {
                ...
                logger.info({ propertyId, count: placesRows.length }, 'properties.nearby_places.inserted');
            }
        }
    ...
    // ─────────────────────────────────────────────────────────────────────────
    async createRentalUnit(propertyId, input) {
    ...
    async createShortTermConfig(propertyId, input) {
        const { error } = await supabaseAdmin
    ...
    async createCommercialConfig(propertyId, input) {
        const { error } = await supabaseAdmin
    ...
    async createPlotDetails(propertyId, input) {
        const { error } = await supabaseAdmin
            .from('plot_details')
            .insert({
            property_id: propertyId,
    ...
    async createOffplanDetails(propertyId, input) {
        const { error } = await supabaseAdmin
    ...
    async listProperties(filters) {
        const { page, limit, listing_category, listing_type, status, county, area, min_price, max_price, bedrooms, is_furnished, is_featured, construction_status, lat, lng, radius, } = filters;
        const from = (page - 1) * limit;
        const to = from + limit - 1;
        let query = supabaseAdmin
            .from('properties')
            .select(`id, listing_category, listing_type, title, status, bedrooms, bathrooms,
         ...
         listing_search_scores ( total_score )`, { count: 'exact' })
            .is('deleted_at', null)
    ...
    // UPDATE
    // ─────────────────────────────────────────────────────────────────────────
    async updateProperty(propertyId, userId, input, isAdmin = false) {
        // Ownership check (admins bypass)
        if (!isAdmin) {
            await this.assertOwnership(propertyId, userId);
        }
        const { location, pricing, contacts, media, amenities, nearby_places, rental_unit, short_term_config, commercial_config, plot_details, offplan_details, ...coreUpdates } = input;
        // ── Core fields ──────────────────────────────────────────────────────
        ...
        if (location) {
            ...
            if (error)
                logger.error({ error, propertyId }, 'properties.location.update.failed');
        }
        // ── Pricing ──────────────────────────────────────────────────────────
        if (pricing) {
            const { error } = await supabaseAdmin
                ...
                .eq('property_id', propertyId);
            if (error)
                logger.error({ error, propertyId }, 'properties.pricing.update.failed');
        }
        // ── Contacts — full replace if provided ──────────────────────────────
        if (contacts && contacts.length > 0) {
            await supabaseAdmin.from('property_contacts').delete().eq('property_id', propertyId);
            await supabaseAdmin.from('property_contacts').insert(contacts.map((c) => ({ property_id: propertyId, ...c })));
        }
        ...
        if (media && media.length > 0) {
            ...
            if (existingMedia && existingMedia.length > 0) {
                await Promise.allSettled(existingMedia.map((m) => deletePropertyMedia(extractCloudinaryPublicId(m.url), (m.media_type === 'video' || m.media_type === 'drone') ? 'video' : 'image')));
            }
            ...
            await supabaseAdmin.from('property_media').insert(uploaded.map((result, idx) => ({
                property_id: propertyId,
                media_type: media[idx].media_type,
                url: result.url,
                thumbnail_url: result.thumbnail_url,
        ...
        if (amenities && amenities.length > 0) {
            ...
            await supabaseAdmin.from('property_amenities').insert(amenities.map((a) => ({ property_id: propertyId, ...a })));
        }
        ...
        if (nearby_places && nearby_places.length > 0) {
            ...
            if (propLat && propLon) {
                ...
                await supabaseAdmin.from('nearby_places').insert(nearby_places.map((place) => {
                    const { distanceM, walkMinutes, driveMinutes } = computeDistanceResult(propLat, propLon, place.latitude, place.longitude);
                    return {
                        property_id: propertyId,
                        place_type: place.place_type,
    ...
    // ─────────────────────────────────────────────────────────────────────────
    async upsertRentalUnit(propertyId, input) {
        if (input === null) {
            // Delete if exists
    ...
    async upsertShortTermConfig(propertyId, input) {
        if (input === null) {
    ...
    async upsertCommercialConfig(propertyId, input) {
        if (input === null) {
    ...
    async upsertPlotDetails(propertyId, input) {
        if (input === null) {
    ...
    async upsertOffplanDetails(propertyId, input) {
        if (input === null) {
    ...
    // ─────────────────────────────────────────────────────────────────────────
    async addAvailabilityBlocks(propertyId, userId, blocks, isAdmin = false) {
        if (!isAdmin)
            await this.assertOwnership(propertyId, userId);
        ...
        const { data, error } = await supabaseAdmin
            ...
            .select();
        if (error)
            throw new Error(`Failed to add availability blocks: ${error.message}`);
        return data;
    }
    ...
    async deleteAvailabilityBlock(blockId, propertyId, userId, isAdmin = false) {
        if (!isAdmin)
            await this.assertOwnership(propertyId, userId);
        const { error } = await supabaseAdmin
            .from('availability_calendar')
    ...
    async getBuildingById(buildingId) {
      ...
      `)
            ...
            .single();
        if (error)
            throw new Error(`Building not found: ${error.message}`);
        return data;
    }
    ...
    // SOFT DELETE
    // ─────────────────────────────────────────────────────────────────────────
    async deleteProperty(propertyId, userId, isAdmin = false) {
        if (!isAdmin) {
            await this.assertOwnership(propertyId, userId);
        }
        const { error } = await supabaseAdmin
        ...
        deletePropertyMediaByTag(propertyId).catch((err) => logger.error({ err, propertyId }, 'cloudinary.delete_on_property_delete.failed'));
    ...
    // OWNER — my properties
    // ─────────────────────────────────────────────────────────────────────────
    async getMyProperties(userId, filters = {}) {
    ...
    async getAllPropertiesAdmin(page = 1, limit = 20) {
        ...
        const { data, count, error } = await supabaseAdmin
            .from('properties')
            .select(`id, listing_category, listing_type, title, status, created_by,
         created_at, deleted_at,
         property_locations ( county, area ),
         property_pricing   ( asking_price, monthly_rent, currency )`, { count: 'exact' })
            .order('created_at', { ascending: false })
    ...
    // ─────────────────────────────────────────────────────────────────────────
    async uploadMediaForProperty(propertyId, userId, mediaItems, isAdmin = false) {
        if (!isAdmin)
            await this.assertOwnership(propertyId, userId);
        ...
        const uploaded = await uploadPropertyMediaBatch(mediaItems.map((m, idx) => ({
            fileSource: m.file,
            options: {
                mediaType: m.media_type,
                propertyId,
        ...
        logger.info({ propertyId, userId, count: uploaded.length }, 'properties.media.upload_to_cloudinary.success');
        return data;
    }
    async deleteMediaItem(mediaId, propertyId, userId, isAdmin = false) {
        if (!isAdmin)
            await this.assertOwnership(propertyId, userId);
        const { data: mediaRow, error: fetchErr } = await supabaseAdmin
            ...
            .eq('property_id', propertyId)
            .maybeSingle();
        if (fetchErr || !mediaRow)
            throw new Error('Media item not found');
        // Delete from Cloudinary (best-effort)
        const resourceType = mediaRow.media_type === 'video' || mediaRow.media_type === 'drone' ? 'video' : 'image';
        ...
        const { error: delErr } = await supabaseAdmin
            ...
            .eq('property_id', propertyId);
        if (delErr)
            throw new Error(`Failed to delete media: ${delErr.message}`);
        return { success: true };
    }
    // ─────────────────────────────────────────────────────────────────────────
    // NEARBY PLACES — add to existing property
    // ─────────────────────────────────────────────────────────────────────────
    async addNearbyPlaces(propertyId, userId, places, isAdmin = false) {
        if (!isAdmin)
            await this.assertOwnership(propertyId, userId);
        const { data: loc } = await supabaseAdmin
            .from('property_locations')
        ...
        const rows = places.map((place) => {
            const { distanceM, walkMinutes, driveMinutes } = computeDistanceResult(loc.latitude, loc.longitude, place.latitude, place.longitude);
            return {
                property_id: propertyId,
                place_type: place.place_type,
                name: place.name,
        ...
        const { data, error } = await supabaseAdmin
            ...
            .select();
        if (error)
            throw new Error(`Failed to add nearby places: ${error.message}`);
        return data;
    }
    async deleteNearbyPlace(placeId, propertyId, userId, isAdmin = false) {
        if (!isAdmin)
            await this.assertOwnership(propertyId, userId);
        const { error } = await supabaseAdmin
            .from('nearby_places')
            ...
            .eq('property_id', propertyId);
        if (error)
            throw new Error(`Failed to delete nearby place: ${error.message}`);
        return { success: true };
    }
    // ─────────────────────────────────────────────────────────────────────────
    // Private helpers
    // ─────────────────────────────────────────────────────────────────────────
    async assertOwnership(propertyId, userId) {
        const { data, error } = await supabaseAdmin
            .from('properties')
            .select('created_by')
            .eq('id', propertyId)
    ...
    _buildProperty(raw) {
        ...
        return {
            ...
            media: (raw.property_media ?? []).sort((a, b) => a.sort_order - b.sort_order),
            amenities: raw.property_amenities ?? [],
            nearby_places: (raw.nearby_places ?? []).sort((a, b) => a.distance_m - b.distance_m),
            score: score ? {
