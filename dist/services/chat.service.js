...
export class ChatService {
     ...
     */
    async startConversation(initiatorId, input) {
        ...
        const { data: existing } = await supabaseAdmin
            ...
            .or(`and(participant_a.eq.${initiatorId},participant_b.eq.${recipient_id}),` +
     ...
     */
    async getMyConversations(userId, page = 1, limit = 30) {
    ...
    async getConversationById(convId, requestingUserId) {
    ...
    async archiveConversation(convId, userId) {
    ...
    async blockConversation(convId, userId) {
        const { data: conv } = await supabaseAdmin
            .from('conversations').select('participant_a, participant_b').eq('id', convId).maybeSingle();
        if (!conv)
            throw new Error('Conversation not found');
    ...
    // MESSAGES
    // ─────────────────────────────────────────────────────────────────────────
    async sendMessage(senderId, convId, input) {
     ...
     */
    async getMessages(convId, userId, page = 1, limit = 50) {
        const { data: conv } = await supabaseAdmin
            .from('conversations')
      ...
      `, { count: 'exact' })
        ...
        const sanitised = (data ?? []).map((m) => m.is_deleted
    ...
    async deleteMessage(messageId, senderId) {
        ...
        if (msg.sender_id !== senderId)
            throw new Error('Forbidden: you did not send this message');
        if (msg.is_deleted)
    ...
    async reportMessage(messageId, reportedBy, reason) {
    ...
    async _markRead(convId, userId, conv) {
        await supabaseAdmin
            .from('messages')
            .update({ read_by_recipient: true, read_at: new Date().toISOString() })
            .eq('conversation_id', convId)
