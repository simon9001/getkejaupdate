...
export class ReviewsService {
    ...
    // SUBMIT REVIEW
    // ─────────────────────────────────────────────────────────────────────────
    async submitReview(reviewerId, input, meta) {
        const { review_type, visit_id, long_term_booking_id, short_stay_booking_id, rating_overall, review_text, ...ratings } = input;
        ...
        if (interaction.completedAt) {
            const hoursSince = (Date.now() - new Date(interaction.completedAt).getTime()) / 3_600_000;
            if (hoursSince < 48) {
                throw new Error(`Reviews can be submitted 48 hours after the interaction completes. ` +
        ...
        const initialStatus = highSignals.length > 0
            ? 'held_for_moderation'
            : interaction.isMutualBlind
                ? 'blind_pending'
                : 'pending';
        const autoPublishAt = initialStatus === 'pending'
        ...
        if (error) {
            ...
            if (error.code === '23505')
                throw new Error('You have already reviewed this interaction');
            throw new Error(`Failed to submit review: ${error.message}`);
        }
        // ── 6. Store fraud signals ────────────────────────────────────────────
        if (signals.length > 0) {
            await supabaseAdmin.from('review_fraud_signals').insert(signals.map((s) => ({
                review_id: review.id,
                signal: s.signal,
        ...
        logger.info({ reviewId: review.id, reviewerId, status: initialStatus, signals: signals.length }, 'review.submitted');
        return {
            review_id: review.id,
            status: initialStatus,
            auto_publish_at: autoPublishAt,
            fraud_signals: signals.length,
            message: initialStatus === 'held_for_moderation'
    ...
    // EDIT REVIEW
    // ─────────────────────────────────────────────────────────────────────────
    async editReview(reviewId, reviewerId, updates) {
        ...
        if (hoursSince > 48)
            throw new Error('Reviews can only be edited within 48 hours of submission');
        if (review.edit_count >= 3)
            throw new Error('Maximum edit limit (3) reached for this review');
        ...
        if (updates.rating_overall !== undefined)
            patch.rating_overall = updates.rating_overall;
        if (updates.review_text !== undefined)
            patch.review_text = updates.review_text;
    ...
    // ─────────────────────────────────────────────────────────────────────────
    async replyToReview(reviewId, replierUserId, replyText) {
        const { data: review } = await supabaseAdmin
            .from('unified_reviews')
        ...
        if (!review)
            throw new Error('Review not found');
        if (review.reviewee_id !== replierUserId)
            throw new Error('Forbidden: you are not the reviewee');
        if (review.status !== 'published')
            throw new Error('Can only reply to published reviews');
    ...
    // ADMIN MODERATION
    // ─────────────────────────────────────────────────────────────────────────
    async moderateReview(reviewId, adminId, action, notes) {
        const statusMap = {
            approve: 'published',
            reject: 'rejected',
    ...
    async resolveSignal(signalId, adminId) {
        await supabaseAdmin
    ...
    // ─────────────────────────────────────────────────────────────────────────
    async _resolveInteraction(opts) {
        ...
        if (long_term_booking_id) {
            ...
            const isMutual = review_type === 'tenant_reviews_landlord' || review_type === 'landlord_reviews_tenant';
            let pairReviewId;
            if (isMutual) {
        ...
        if (short_stay_booking_id) {
            const { data } = await supabaseAdmin
                ...
                .maybeSingle();
            if (!data)
                throw new Error('Short-stay booking not found');
    ...
    // ─────────────────────────────────────────────────────────────────────────
    async _checkVelocity(reviewerId) {
        ...
        if ((count ?? 0) >= 5)
            return { signal: 'velocity_multiple_reviews', confidence: 'high', detail: `${count} reviews in last 24h` };
        if ((count ?? 0) >= 3)
            return { signal: 'velocity_multiple_reviews', confidence: 'medium', detail: `${count} reviews in last 24h` };
        return null;
    }
    async _checkAccountAge(ageDays) {
        if (ageDays < 3)
            return { signal: 'account_age_too_new', confidence: 'high', detail: `Account is ${ageDays} day(s) old` };
        if (ageDays < 7)
            return { signal: 'account_age_too_new', confidence: 'medium', detail: `Account is ${ageDays} day(s) old` };
        if (ageDays < 14)
            return { signal: 'account_age_too_new', confidence: 'low', detail: `Account is ${ageDays} day(s) old` };
        return null;
    }
    async _checkTextDuplicate(reviewText) {
        ...
        for (const r of recent ?? []) {
            ...
            if (sim >= 0.9)
                return { signal: 'text_duplicate', confidence: 'high', detail: `${Math.round(sim * 100)}% similarity to existing review` };
            if (sim >= 0.8)
                return { signal: 'text_duplicate', confidence: 'medium', detail: `${Math.round(sim * 100)}% similarity to existing review` };
        }
        return null;
    }
    async _checkReciprocalPattern(reviewerId, revieweeId) {
        if (!revieweeId)
            return null;
        ...
        const { data } = await supabaseAdmin
            ...
            .maybeSingle();
        if (data)
            return { signal: 'reciprocal_pattern', confidence: 'high', detail: 'Mutual 5-star exchange within 72h' };
        return null;
    }
    async _checkIpCluster(ip) {
        if (!ip)
            return null;
        ...
        if ((count ?? 0) >= 5)
            return { signal: 'ip_cluster', confidence: 'high', detail: `${count} reviews from this IP in 24h` };
        if ((count ?? 0) >= 3)
            return { signal: 'ip_cluster', confidence: 'medium', detail: `${count} reviews from this IP in 24h` };
        return null;
    }
    async _countPriorReviews(reviewerId) {
        const { count } = await supabaseAdmin
            .from('unified_reviews')
            .select('id', { count: 'exact', head: true })
            .eq('reviewer_id', reviewerId);
