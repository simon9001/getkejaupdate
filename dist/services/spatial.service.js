...
export class SpatialService {
     ...
     */
    async searchExternalPlaces(query) {
        try {
     ...
     */
    async linkLandmark(propertyId, landmark) {
        ...
        if (distanceError) {
            const isMissing = distanceError.message.includes('not found') ||
                distanceError.code === 'PGRST202';
            logger.error({ error: distanceError, propertyId, landmarkId }, 'Distance calculation failed');
            if (isMissing) {
                throw new Error('Database function "calculate_property_landmark_distance" is missing. ' +
    ...
    normalizeLandmarkType(type) {
        ...
        if (t.includes('shopping') || t.includes('centre') ||
            t.includes('center') || t.includes('mall'))
            return 'shopping_center';
        if (t.includes('market') || t.includes('shop') ||
            t.includes('retail') || t.includes('store'))
            return 'market';
        ...
        if (t.includes('road') || t.includes('highway') ||
            t.includes('street') || t.includes('avenue'))
            return 'road';
