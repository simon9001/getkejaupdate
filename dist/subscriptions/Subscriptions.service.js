...
export class SubscriptionsService {
    ...
    async getMySubscription(userId) {
      ...
      `)
        ...
        const creditsRemaining = Math.max(0, (plan?.viewing_unlocks_per_month ?? 0) - (data.unlock_credits_used ?? 0));
    ...
    async subscribe(userId, input) {
        ...
        if (existing) {
            ...
            const existingPlanName = existing.plan?.name ?? 'unknown plan';
            throw new Error(`You already have an active subscription (${existingPlanName}). Cancel it first or use the upgrade endpoint.`);
        }
        // ── Resolve plan + price ──────────────────────────────────────────────
        const plan = await this.getPlanById(plan_id);
        const amountKes = billing_cycle === 'annual'
     ...
     */
    async upgradePlan(userId, input) {
        ...
        await supabaseAdmin
            ...
            .eq('id', current.id);
        logger.info({ userId, previousSubId: current.id }, 'subscription.upgrade.previous_cancelled');
    ...
    async cancelSubscription(userId, input) {
        ...
        logger.info({ userId, subId: current.id, reason: input.reason }, 'subscription.cancelled');
        return {
      ...
      `)
        ...
        try {
            await this._simulatePayment({
                ...
                paymentMethod: 'mpesa',
                description: `Renewal: ${sub.subscription_plans?.name ?? sub.plan?.name ?? 'plan'}`,
            });
            const newRenewsAt = this._computeRenewalDate(new Date(sub.renews_at), sub.billing_cycle);
            await supabaseAdmin
                .from('user_subscriptions')
                .update({
     ...
     */
    async getViewingFee(userId, propertyId) {
      ...
      `)
        ...
        if (subscription?.status === 'active' && !isFreeForThisUser) {
            ...
            const creditsLeft = (plan?.viewing_unlocks_per_month ?? 0) - (subscription.unlock_credits_used ?? 0);
            if (creditsLeft > 0)
                useCredit = true;
        }
     ...
     */
    async unlockProperty(userId, input) {
        ...
        logger.info({ userId, propertyId: property_id, fee: feeInfo.fee_kes, useCredit: feeInfo.use_credit }, 'viewing.unlocked');
        return {
     ...
     */
    async bookViewing(userId, input) {
        ...
        const { error: updateErr } = await supabaseAdmin
            ...
            .eq('id', unlock.id);
        if (updateErr)
            throw new Error(`Failed to book viewing: ${updateErr.message}`);
    ...
    async getMyUnlocks(userId) {
      ...
      `)
        ...
        if (error)
            throw new Error(`Failed to fetch unlocks: ${error.message}`);
        return data ?? [];
    }
    // ─────────────────────────────────────────────────────────────────────────
    // ADMIN
    ...
    async getAllSubscriptions(page = 1, limit = 20, status) {
        ...
        let query = supabaseAdmin
            .from('user_subscriptions')
            .select(`id, user_id, billing_cycle, amount_kes, status,
         started_at, renews_at, cancelled_at, unlock_credits_used,
         subscription_plans ( id, name, price_monthly_kes ),
         users ( id, email )`, { count: 'exact' })
            .order('started_at', { ascending: false })
     ...
     */
    async adminSetStatus(subscriptionId, input) {
        ...
        logger.info({ subscriptionId, status: input.status, reason: input.reason }, 'subscription.admin.status_override');
        return data;
    }
    /**
     ...
     * Annual:  +1 year.
     */
    _computeRenewalDate(from, cycle) {
     ...
     */
    _mapListingCategoryToFeeCategory(listingCategory) {
        const map = {
            for_sale: 'for_sale',
