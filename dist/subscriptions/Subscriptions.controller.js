...
function resolveStatus(err) {
    ...
    if (msg.includes('forbidden') || msg.includes('unauthorized'))
        return 403;
    if (msg.includes('already') || msg.includes('already on this plan'))
        return 409;
    if (msg.includes('expired') || msg.includes('past due'))
        return 422;
...
function fail(c, err, code) {
    ...
    logger.error({ requestId: c.get('requestId'), code, message: error.message }, 'subscriptions.controller.error');
...
export class SubscriptionsController {
    ...
    async subscribe(c) {
        try {
            ...
            return c.json({ message: 'Subscription created successfully', code: 'SUBSCRIBED', subscription: result }, 201);
        }
        catch (err) {
