...
export class AuthService {
    ...
    async getActiveSessions(userId, currentAccessToken) {
    ...
    async refreshToken(refreshToken) {
        ...
        const userWithRoles = {
            ...
            roles: tokenData.profile.roles?.map((r) => r.role) || ['user'],
            password_hash: '',
            email_verified: true,
            created_at: new Date(),
            updated_at: new Date()
    ...
    async logSecurityEvent(userId, event, userAgent, ipAddress, metadata) {
        try {
            await supabaseAdmin
                .from('security_logs')
                .insert({
