...
// ---------------------------------------------------------------------------
const googleClient = new OAuth2Client(env.google.clientId, env.google.clientSecret, env.google.redirectUri);
// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------
...
async function fetchUserRoles(userId) {
    const { data } = await supabaseAdmin
        ...
        .eq('user_id', userId)
        .eq('is_active', true);
    return (data ?? []).map((r) => (r.roles?.name ?? 'seeker'));
}
/**
...
async function auditLog(userId, eventType, userAgent, ipAddress, metadata) {
    try {
...
async function assignRole(userId, role, opts = {}) {
    ...
    if (VERIFIED_ROLES.includes(role) && !opts.bypassVerificationCheck) {
...
export class AuthService {
    ...
    // GOOGLE CALLBACK
    // -------------------------------------------------------------------------
    async handleGoogleCallback(code, userAgent, ipAddress) {
        ...
        if (!user) {
            ...
            if (createError || !newUser)
                throw createError ?? new Error('Failed to create Google user');
            await supabaseAdmin
                .from('user_profiles')
        ...
        if (!user)
            throw new Error('Unexpected: user is null after Google OAuth resolution');
        if (user.account_status === 'banned') {
            return { success: false, error: 'Account has been suspended.', code: 'ACCOUNT_BANNED' };
        }
        await supabaseAdmin
    ...
    async verifyEmail(token) {
        ...
        if (user.email_verified)
            return { success: true, message: 'Email already verified.', code: 'ALREADY_VERIFIED' };
    ...
    // CHANGE PASSWORD
    // -------------------------------------------------------------------------
    async changePassword(userId, currentPass, newPass, userAgent, ipAddress) {
        const { data: user } = await supabaseAdmin
            .from('users')
    ...
    async refreshToken(refreshToken) {
        ...
        if (!session || new Date(session.expires_at) < new Date()) {
            ...
            if (replayedSession) {
                ...
                await auditLog(replayedSession.user_id, 'token_reuse_detected', undefined, undefined, { revokedSessionId: replayedSession.id });
            }
        ...
        const userWithRoles = { userId: session.user_id, email: user?.email ?? '', roles };
        const accessToken = tokenService.generateAccessToken(userWithRoles);
    ...
    // LOGOUT
    // -------------------------------------------------------------------------
    async logout(userId, refreshToken, logoutAll = false, userAgent, ipAddress) {
        if (logoutAll) {
            await supabaseAdmin
                .from('user_sessions')
                .update({ is_active: false, revoked_at: new Date().toISOString() })
    ...
    // LOGOUT OTHERS
    // -------------------------------------------------------------------------
    async logoutOthers(userId, currentRefreshToken, userAgent, ipAddress) {
    ...
    // REVOKE SESSION
    // -------------------------------------------------------------------------
    async revokeSession(userId, sessionId, userAgent, ipAddress) {
        const { data: session } = await supabaseAdmin
            .from('user_sessions')
            .select('id')
            .eq('id', sessionId)
