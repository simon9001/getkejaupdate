...
}).refine((d) => d.body || d.media_url, { message: 'Message must have body or media_url' });
...
}).refine((d) => new Date(d.proposed_datetime) > new Date(), { message: 'Proposed visit time must be in the future', path: ['proposed_datetime'] });
...
}).refine((d) => new Date(d.desired_move_in) >= new Date(new Date().toISOString().split('T')[0]), { message: 'Move-in date cannot be in the past', path: ['desired_move_in'] });
...
}).refine((d) => d.payment_method !== 'mpesa' || !!d.mpesa_phone, { message: 'mpesa_phone required for M-Pesa', path: ['mpesa_phone'] });
...
}, { message: 'Exactly one of visit_id, long_term_booking_id, short_stay_booking_id is required' }).refine(
// Extreme ratings without text are suspicious — require text for 1 or 5 stars
(d) => (d.rating_overall >= 2 && d.rating_overall <= 4) || (d.review_text && d.review_text.length >= 20), { message: 'Reviews with 1 or 5 stars must include a written review (min 20 characters)', path: ['review_text'] });
