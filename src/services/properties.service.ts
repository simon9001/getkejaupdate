...
import type { CreatePropertyInput, Property, PropertyStatus } from '../types/property.types.js';

export class PropertiesService {
    /**
     * Create a new property with its location
     */
    async createProperty(ownerId: string, input: CreatePropertyInput): Promise<Property> {
        const {
            address, town, county, latitude, longitude,
            neighborhood, amenity_ids, images,
            ...propertyData
        } = input;
    ...
    async getPropertyById(id: string): Promise<Property> {
      ...
      `)
        ...
        if (property.amenities) {
            property.amenities = property.amenities.map((a: any) => a.amenity);
        }
