...
export class VisitsService {
  ...
  async requestVisit(seekerUserId: string, input: RequestVisitInput) {
    ...
    if (property.status !== 'available') throw new Error('Property is not currently available for viewings');
    if (property.created_by === seekerUserId) throw new Error('You cannot schedule a visit to your own property');
    ...
    if (error) throw new Error(`Failed to request visit: ${error.message}`);
  ...
  async confirmVisit(visitId: string, hostUserId: string, input: ConfirmVisitInput) {
  ...
  async rescheduleVisit(visitId: string, userId: string, input: RescheduleVisitInput) {
  ...
  async cancelVisit(visitId: string, userId: string, reason: string) {
    const { data: visit } = await supabaseAdmin
      .from('visit_schedules')
  ...
  async completeVisit(visitId: string, userId: string, outcomeNotes?: string) {
  ...
  async markNoShow(visitId: string, reportedByUserId: string) {
    const { data: visit } = await supabaseAdmin
      .from('visit_schedules')
  ...
  async getMyVisitsAsSeeker(userId: string, status?: string) {
    let q = supabaseAdmin
      .from('visit_schedules')
      .select(`
  ...
  async getMyVisitsAsHost(userId: string, status?: string) {
    let q = supabaseAdmin
      .from('visit_schedules')
      .select(`
        id, proposed_datetime, confirmed_datetime, status, visit_type,
    ...
    const { data, error } = await q;
    if (error) throw new Error(`Failed to fetch host visits: ${error.message}`);
    return data ?? [];
  }
  ...
  private async _fetchAndAssertHost(visitId: string, hostUserId: string) {
    const { data: visit } = await supabaseAdmin
      .from('visit_schedules')
    ...
    if (!visit) throw new Error('Visit not found');
    if (visit.host_user_id !== hostUserId) throw new Error('Forbidden: you are not the host of this visit');
    return visit;
  }

  private _assertParticipant(visit: any, userId: string) {
