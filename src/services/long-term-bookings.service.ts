...
export class LongTermBookingsService {
  ...
  async applyForTenancy(tenantUserId: string, input: ApplyLongTermInput) {
    ...
    if (property.created_by === tenantUserId) throw new Error('Cannot apply for your own property');
  ...
  async approveApplication(bookingId: string, landlordUserId: string, input: ApproveLongTermInput) {
  ...
  async rejectApplication(bookingId: string, landlordUserId: string, reason: string) {
    const booking = await this._fetchAndAssertLandlord(bookingId, landlordUserId);
  ...
  async payDeposit(bookingId: string, tenantUserId: string, input: PayDepositInput) {
    ...
    if (Math.abs(input.amount_paid_kes - booking.agreed_deposit_kes) > tolerance) {
      throw new Error(
        `Amount paid (KES ${input.amount_paid_kes}) does not match the agreed deposit (KES ${booking.agreed_deposit_kes})`,
      );
    }
  ...
  async activateTenancy(bookingId: string, landlordUserId: string) {
    const booking = await this._fetchAndAssertLandlord(bookingId, landlordUserId);
  ...
  async giveNotice(bookingId: string, userId: string, noticeDate: string, reason: string) {
  ...
  async terminateTenancy(bookingId: string, userId: string, terminationDate: string, reason: string) {
    const { data: booking } = await supabaseAdmin
      .from('long_term_bookings')
      .select('id, tenant_user_id, landlord_user_id, status, property_id')
      .eq('id', bookingId)
  ...
  async getMyApplicationsAsTenant(tenantUserId: string, status?: string) {
    let q = supabaseAdmin
      .from('long_term_bookings')
      .select(`
  ...
  async getMyApplicationsAsLandlord(landlordUserId: string, status?: string) {
    let q = supabaseAdmin
      .from('long_term_bookings')
      .select(`
    ...
    if (status) q = q.eq('status', status);
    const { data, error } = await q;
    if (error) throw new Error(`Failed to fetch landlord applications: ${error.message}`);
    return data ?? [];
  }
  ...
  private async _fetchAndAssertLandlord(bookingId: string, landlordUserId: string) {
    ...
    if (data.landlord_user_id !== landlordUserId) throw new Error('Forbidden: you are not the landlord on this booking');
    return data;
  }

  private async _fetchAndAssertTenant(bookingId: string, tenantUserId: string) {
    const { data } = await supabaseAdmin
      .from('long_term_bookings')
      .select('id, booking_ref, property_id, tenant_user_id, landlord_user_id, status, agreed_deposit_kes')
      .eq('id', bookingId)
    ...
    if (!data) throw new Error('Booking not found');
    if (data.tenant_user_id !== tenantUserId) throw new Error('Forbidden: you are not the tenant on this booking');
    return data;
  }

  private _assertParticipant(booking: any, userId: string) {
