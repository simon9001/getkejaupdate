...
export interface NearbyParams extends SearchParams {
  lat:        number;
  lng:        number;
  radius_km?: number;
}
...
function rowCoords(row: any): { lat: number; lng: number } | null {
...
export class SearchService {
  ...
  private async fetchCandidates(params: SearchParams, fetchLimit: number) {
    const {
      q, listing_category, listing_type, county, area,
      min_price, max_price, bedrooms, is_furnished,
      is_featured, construction_status,
      page = 1, limit = 20,
    } = params;
  ...
  // ── 2. Radius search ───────────────────────────────────────────────────────
  async searchNearby(params: NearbyParams) {
    ...
    const { data, error } = await this.fetchCandidates(
      { ...filters, page: 1, limit: MAX_CANDIDATES },
      MAX_CANDIDATES,
    );
    ...
    const withDist = (data ?? [])
      ...
      .filter((r): r is NonNullable<typeof r> => r !== null && r.distance_m <= radiusM)
  ...
  async searchInBounds(params: BoundsParams) {
    ...
    const inBounds = (data ?? []).filter((row: any) => {
      const coords = rowCoords(row);
