...
const uid = (c: any): string => c.get('user')?.userId as string;
...
chatRouter.post('/start', authenticate, async (c) => {
  try {
    ...
    return c.json({ message: err.message ?? 'Failed to start conversation', code: 'CHAT_ERROR' }, status as any);
  }
});
...
chatRouter.get('/conversations/:id', authenticate, async (c) => {
  try {
    ...
    return c.json({ message: err.message ?? 'Failed to fetch conversation', code: 'CHAT_ERROR' }, status as any);
  }
});
...
chatRouter.get('/conversations/:id/messages', authenticate, async (c) => {
  try {
    ...
    const status = err.message?.includes('Forbidden') ? 403 : err.message?.includes('not found') ? 404 : 500;
    return c.json({ message: err.message ?? 'Failed to fetch messages', code: 'CHAT_ERROR' }, status as any);
  }
});
...
chatRouter.post('/conversations/:id/messages', authenticate, async (c) => {
  try {
    ...
    return c.json({ message: err.message ?? 'Failed to send message', code: 'CHAT_ERROR' }, status as any);
  }
});
...
chatRouter.delete('/messages/:id', authenticate, async (c) => {
  try {
    ...
    return c.json({ message: err.message ?? 'Failed to delete message', code: 'CHAT_ERROR' }, status as any);
  }
});
...
chatRouter.post('/messages/:id/report', authenticate, async (c) => {
  try {
    ...
    if (!reason) return c.json({ message: 'reason is required', code: 'VALIDATION_ERROR' }, 400);
