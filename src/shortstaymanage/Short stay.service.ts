...
import type {
  ...
  CheckinInput,
  CancelBookingInput,
  SubmitPropertyReviewInput,
  SubmitGuestReviewInput,
  RaiseDisputeInput,
  ResolveDisputeInput,
  ListBookingsQuery,
  CancellationPolicy,
} from '../types/short-stay.types.js';

// =============================================================================
// Refund policy calculator
// =============================================================================

function calculateRefundPct(
  policy: CancellationPolicy,
  nightsBeforeCheckin: number,
): number {
  switch (policy) {
    case 'flexible':
...
export class ShortStayService {
  ...
  async checkAvailability(
    ...
    checkIn: string,
    checkOut: string,
  ): Promise<{ available: boolean; blockedDates: string[]; reason?: string }> {
   ...
   */
  async getPriceQuote(propertyId: string, checkIn: string, checkOut: string, guestsCount: number) {
    const nights = nightsBetween(checkIn, checkOut);
    if (nights <= 0) throw new Error('check_out_date must be after check_in_date');
    ...
    if (!config?.price_per_night) throw new Error('Property pricing not configured');
   ...
   */
  async createBooking(guestUserId: string, input: CreateBookingInput) {
    const {
      property_id, check_in_date, check_out_date,
      guests_count, special_requests, payment_method,
      mpesa_phone, guest_name, guest_phone,
    } = input;
    ...
    if (!property?.created_by) throw new Error('Property host not found');
   ...
   */
  async _onPaymentSuccess(
    bookingId: string,
    paymentRef: string,
    quote: Awaited<ReturnType<typeof this.getPriceQuote>>,
  ) {
   ...
   */
  async recordCheckin(bookingId: string, performedBy: string, input: CheckinInput) {
    ...
    if (checkinErr) throw new Error(`Failed to record ${event_type}: ${checkinErr.message}`);
    ...
    logger.info(
      { bookingId, eventType: event_type, performedBy, autoVerify },
      'booking.checkin.recorded',
    );
   ...
   */
  async confirmCheckin(bookingId: string, confirmedBy: string) {
    ...
    if (!checkinRow) throw new Error('No check-in record found for this booking');
    if (checkinRow.verified) throw new Error('Check-in is already verified');
   ...
   */
  private async _releaseHostPayout(booking: any, checkinId: string) {
    ...
    logger.info(
      { bookingId: booking.id, hostId: booking.host_user_id, amount: booking.host_payout_kes },
   ...
   */
  async cancelBooking(bookingId: string, cancelledByUserId: string, input: CancelBookingInput) {
    const { data: booking } = await supabaseAdmin
      .from('short_stay_bookings')
    ...
    if (!isGuest && !isHost) throw new Error('Forbidden: you are not part of this booking');

    // Calculate refund
    const nightsBeforeCheckin = nightsBetween(
    ...
    if (isHost) {
      ...
      refundPct = calculateRefundPct(booking.cancellation_policy as CancellationPolicy, nightsBeforeCheckin);
    ...
    logger.info(
      { bookingId, cancelledBy: isGuest ? 'guest' : 'host', refundPct, refundAmount },
      'booking.cancelled',
    );
  ...
  async getBookingById(bookingId: string, requestingUserId?: string) {
    ...
    if (requestingUserId === data.guest_user_id) {
      const { host_payout_kes, ...safeData } = data as any;
      return safeData;
    }
  ...
  async getMyBookingsAsGuest(userId: string, query: ListBookingsQuery) {
    ...
    if (from_date) q = q.gte('check_in_date', from_date);
    if (to_date)   q = q.lte('check_out_date', to_date);
  ...
  async getMyBookingsAsHost(userId: string, query: ListBookingsQuery) {
    const { page, limit, status, from_date, to_date } = query;
    const from = (page - 1) * limit;
    ...
    const { data, count, error } = await q;
    if (error) throw new Error(`Failed to fetch host bookings: ${error.message}`);
   ...
   */
  async submitPropertyReview(guestUserId: string, input: SubmitPropertyReviewInput) {
    ...
    if (!booking) throw new Error('Booking not found');
    if (booking.guest_user_id !== guestUserId) throw new Error('Forbidden: this is not your booking');

    const allowedStatuses: string[] = ['checked_out', 'completed'];
   ...
   */
  async submitGuestReview(hostUserId: string, input: SubmitGuestReviewInput) {
    const { booking_id, ...ratings } = input;
    ...
    if (!booking) throw new Error('Booking not found');
    if (booking.host_user_id !== hostUserId) throw new Error('Forbidden: this is not your listing');
    ...
    const daysSinceCheckout = nightsBetween(booking.check_out_date, new Date().toISOString().split('T')[0]);
    if (daysSinceCheckout > 14) throw new Error('Review window has closed (14 days after checkout)');
    ...
    const { data: existing } = await supabaseAdmin
      ...
      .eq('booking_id', booking_id)
      .maybeSingle();
    if (existing) throw new Error('You have already reviewed this guest');
    ...
    if (error) throw new Error(`Failed to submit guest review: ${error.message}`);
    return review;
  }
   ...
   */
  async replyToReview(reviewId: string, hostUserId: string, reply: string) {
    ...
    if (review.host_user_id !== hostUserId) throw new Error('Forbidden: this is not your listing');
    if (review.host_reply) throw new Error('You have already replied to this review');
  ...
  async raiseDispute(raisedByUserId: string, input: RaiseDisputeInput) {
  ...
  async resolveDispute(disputeId: string, adminUserId: string, input: ResolveDisputeInput) {
  ...
  async getAllBookingsAdmin(query: ListBookingsQuery) {
    ...
    const { data, count, error } = await q;
    if (error) throw new Error(`Failed to fetch admin bookings: ${error.message}`);
  ...
  async adminFlagReview(reviewId: string, reason: string) {
    const { error } = await supabaseAdmin
      .from('property_reviews')
  ...
  async adminRemoveReview(reviewId: string) {
    const { error } = await supabaseAdmin
      .from('property_reviews')
      .update({ status: 'removed' })
      .eq('id', reviewId);
    if (error) throw new Error(`Failed to remove review: ${error.message}`);
    return { success: true };
  }
  ...
  private async _initiatePayment(opts: {
    bookingId: string; guestUserId: string; amountKes: number;
    paymentMethod: string; mpesaPhone?: string; description: string;
  }): Promise<string> {
  ...
  private async _simulatePayout(opts: {
    recipientUserId: string; amountKes: number; bookingRef: string;
  }): Promise<string> {
  ...
  private async _simulateRefund(opts: {
    bookingId: string; guestUserId: string; amountKes: number; bookingRef: string;
  }): Promise<string> {
