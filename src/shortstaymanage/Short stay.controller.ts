...
function resolveStatus(err: Error): 400 | 403 | 404 | 409 | 422 | 500 {
  ...
  if (msg.includes('already') || msg.includes('already reviewed'))  return 409;
  if (
    msg.includes('not available') || msg.includes('cannot') ||
...
export class ShortStayController {
  ...
  async checkAvailability(c: Context) {
    ...
    } catch (err) { return fail(c, err, 'AVAILABILITY_CHECK_FAILED'); }
  }
  ...
  async getCalendar(c: Context) {
    ...
    } catch (err) { return fail(c, err, 'CALENDAR_FETCH_FAILED'); }
  }

  /** GET /api/short-stay/quote/:propertyId?check_in=&check_out=&guests= */
  async getPriceQuote(c: Context) {
    try {
      const propertyId   = c.req.param('propertyId');
      const checkIn      = c.req.query('check_in');
      const checkOut     = c.req.query('check_out');
    ...
    } catch (err) { return fail(c, err, 'QUOTE_FETCH_FAILED'); }
  }
  ...
  async createBooking(c: Context) {
    ...
    } catch (err) { return fail(c, err, 'BOOKING_CREATE_FAILED'); }
  }
  ...
  async getBooking(c: Context) {
    ...
    } catch (err) { return fail(c, err, 'BOOKING_FETCH_FAILED'); }
  }

  /** GET /api/short-stay/bookings/my/guest */
  async getMyGuestBookings(c: Context) {
    try {
      const user   = c.get('user');
      const q      = c.req.query();
    ...
    } catch (err) { return fail(c, err, 'GUEST_BOOKINGS_FETCH_FAILED'); }
  }

  /** GET /api/short-stay/bookings/my/host */
  async getMyHostBookings(c: Context) {
    try {
      const user   = c.get('user');
      const q      = c.req.query();
    ...
    } catch (err) { return fail(c, err, 'HOST_BOOKINGS_FETCH_FAILED'); }
  }
  ...
  async cancelBooking(c: Context) {
    ...
    } catch (err) { return fail(c, err, 'BOOKING_CANCEL_FAILED'); }
  }

  // ── Check-in / Check-out ──────────────────────────────────────────────────

  /** POST /api/short-stay/bookings/:id/checkin */
  async recordCheckin(c: Context) {
    try {
      const user   = c.get('user');
      const id     = c.req.param('id');
      const input  = await c.req.json();
    ...
    } catch (err) { return fail(c, err, 'CHECKIN_FAILED'); }
  }
  ...
  async submitPropertyReview(c: Context) {
    ...
    } catch (err) { return fail(c, err, 'PROPERTY_REVIEW_FAILED'); }
  }

  /** POST /api/short-stay/reviews/guest */
  async submitGuestReview(c: Context) {
    try {
      const user   = c.get('user');
      const input  = await c.req.json();
    ...
    } catch (err) { return fail(c, err, 'GUEST_REVIEW_FAILED'); }
  }
  ...
  async replyToReview(c: Context) {
    try {
      ...
      const { reply } = await c.req.json() as { reply: string };
      if (!reply?.trim()) return c.json({ message: 'reply is required', code: 'MISSING_REPLY' }, 400);
      const result = await shortStayService.replyToReview(reviewId, user.userId, reply);
      return c.json({ ...result, code: 'REPLY_POSTED' });
    } catch (err) { return fail(c, err, 'REPLY_FAILED'); }
  }
  ...
  async raiseDispute(c: Context) {
    ...
    } catch (err) { return fail(c, err, 'DISPUTE_RAISE_FAILED'); }
  }
  ...
  async getAllBookingsAdmin(c: Context) {
    try {
      const q      = c.req.query();
    ...
    } catch (err) { return fail(c, err, 'ALL_BOOKINGS_FETCH_FAILED'); }
  }
