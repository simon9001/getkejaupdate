...
export type BookingStatus =
  ...
  | 'checked_out' | 'cancelled_guest' | 'cancelled_host'
  | 'disputed' | 'completed';
...
export type DisputeStatus =
  | 'open' | 'under_review' | 'resolved_guest' | 'resolved_host' | 'escalated';
...
export const createBookingSchema = z.object({
  ...
  (d) => new Date(d.check_in_date) >= new Date(new Date().toISOString().split('T')[0]),
  { message: 'check_in_date cannot be in the past', path: ['check_in_date'] },
).refine(
  (d) => d.payment_method !== 'mpesa' || !!d.mpesa_phone,
  { message: 'mpesa_phone is required when payment_method is mpesa', path: ['mpesa_phone'] },
);
...
export type SubmitPropertyReviewInput = z.infer<typeof submitPropertyReviewSchema>;
export type SubmitGuestReviewInput   = z.infer<typeof submitGuestReviewSchema>;
