 ...
 * Calculate and store nearby landmarks for a property
 */
    async calculateNearbyLandmarks(propertyId: string, latitude: number, longitude: number) {
        try {
            // Get all landmarks
 ...
 * Find a landmark by name
 */
    async findLandmarkByName(name: string): Promise<{ id: string; name: string; latitude: number; longitude: number } | null> {
        try {
 ...
 * Get properties near a specific landmark
 */
    async getPropertiesNearLandmark(landmarkName: string, radius: number = 2000, category?: PropertyCategory) {
        try {
     ...
     */
    private async getPropertiesNearLandmarkFallback(landmark: any, radius: number, category?: PropertyCategory) {
        try {
    ...
    async getCategoryStatistics(): Promise<CategoryStatistics[]> {
        try {
            const categories: PropertyCategory[] = ['commercial', 'residential', 'recreational'];
            ...
            for (const category of categories) {
                ...
                const prices = data?.map(p => p.price_per_month).filter(p => p !== null) as number[];
                ...
                const mostCommonType = Object.entries(typeCount)
                    .sort((a, b) => b[1] - a[1])
                    .map(entry => entry[0])[0] as PropertyType || propertyTypes[0];
     ...
     */
    getPropertyTypesByCategory(category: PropertyCategory): PropertyType[] {
        const categoryMap: Record<PropertyCategory, PropertyType[]> = {
    ...
    async getPropertyById(id: string): Promise<Property> {
        try {
            ...
            return this.formatProperty(property) as Property;
 ...
 * Update property
 */
    async updateProperty(id: string, userId: string, updates: Partial<CreatePropertyInput>) {
        try {
            ...
            if (Object.keys(propertyUpdates).length > 0) {
                ...
                if (propertyUpdates.title !== undefined) updateData.title = propertyUpdates.title;
                if (propertyUpdates.description !== undefined) updateData.description = propertyUpdates.description;
                if (propertyUpdates.property_type !== undefined) updateData.property_type = propertyUpdates.property_type;
                if (propertyUpdates.status !== undefined) updateData.status = propertyUpdates.status;
                if (propertyUpdates.size_sqm !== undefined) updateData.size_sqm = propertyUpdates.size_sqm;
                if (propertyUpdates.bedrooms !== undefined) updateData.bedrooms = propertyUpdates.bedrooms;
                if (propertyUpdates.bathrooms !== undefined) updateData.bathrooms = propertyUpdates.bathrooms;
                if (propertyUpdates.floor_level !== undefined) updateData.floor_level = propertyUpdates.floor_level;
                if (propertyUpdates.furnished_status !== undefined) updateData.furnished_status = propertyUpdates.furnished_status;
                if (propertyUpdates.year_built !== undefined) updateData.year_built = propertyUpdates.year_built;
                if (propertyUpdates.renovation_details !== undefined) updateData.renovation_details = propertyUpdates.renovation_details;
                if (propertyUpdates.internet_speed !== undefined) updateData.internet_speed = propertyUpdates.internet_speed;
                if (propertyUpdates.price_per_month !== undefined) updateData.price_per_month = propertyUpdates.price_per_month;
                if (propertyUpdates.price_per_night !== undefined) updateData.price_per_night = propertyUpdates.price_per_night;
                if (propertyUpdates.currency !== undefined) updateData.currency = propertyUpdates.currency;
                if (propertyUpdates.security_deposit !== undefined) updateData.security_deposit = propertyUpdates.security_deposit;
                if (propertyUpdates.cleaning_fee !== undefined) updateData.cleaning_fee = propertyUpdates.cleaning_fee;
                if (propertyUpdates.service_fee !== undefined) updateData.service_fee = propertyUpdates.service_fee;
                if (propertyUpdates.tax_amount !== undefined) updateData.tax_amount = propertyUpdates.tax_amount;
            ...
            if (address || town || county || (latitude && longitude)) {
                ...
                if (town) locationUpdate.town = town;
                if (county) locationUpdate.county = county;
            ...
            if (neighborhood) {
                const neighborhoodUpdate: any = {};
                if (neighborhood.crime_rating !== undefined) neighborhoodUpdate.crime_rating = neighborhood.crime_rating;
                if (neighborhood.noise_level !== undefined) neighborhoodUpdate.noise_level = neighborhood.noise_level;
                if (neighborhood.community_vibe !== undefined) neighborhoodUpdate.community_vibe = neighborhood.community_vibe;
                if (neighborhood.light_exposure !== undefined) neighborhoodUpdate.light_exposure = neighborhood.light_exposure;
     ...
     */
    async addPropertyImages(propertyId: string, userId: string, images: { url: string; isPrimary?: boolean; sortOrder?: number }[]) {
        try {
            // Verify ownership
     ...
     */
    async deletePropertyImage(imageId: string, userId: string) {
        try {
     ...
     */
    async getPropertiesByOwner(ownerId: string, filters?: { status?: PropertyStatus }) {
        try {
            let query = supabaseAdmin
                .from('properties')
                .select(`
 ...
 * Search by location (legacy method)
 */
    async searchByLocation(lat: number, lng: number, radius: number = 5000) {
        try {
            // Use RPC function
     ...
     */
    private async searchByLocationFallback(lat: number, lng: number, radius: number) {
        try {
     ...
     */
    async boostProperty(id: string, isBoosted: boolean) {
        try {
            const { data, error } = await supabaseAdmin
                .from('properties')
                .update({
     ...
     */
    async strikeProperty(id: string, isStruck: boolean) {
        try {
            const updates: any = {
                is_struck: isStruck,
     ...
     */
    private async verifyOwnership(propertyId: string, userId: string): Promise<boolean> {
        const { data, error } = await supabaseAdmin
            .from('properties')
            .select('owner_id')
            .eq('id', propertyId)
     ...
     * Get category info
     */
    getCategoryInfo(category: PropertyCategory) {
        const infoMap = {
            commercial: {
