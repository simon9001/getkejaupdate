...
export class StaffService {
  ...
  async approveVerification(verificationId: string, staffId: string) {
    ...
    if (updateError) throw new Error(`Failed to approve verification: ${updateError.message}`);
    ...
    if (fetchError) throw new Error(`Failed to fetch verification: ${fetchError.message}`);
  ...
  async rejectVerification(verificationId: string, staffId: string, reason: string) {
  ...
  async getOpenDisputes(page: number, limit: number) {
    const from = (page - 1) * limit;
  ...
  async resolveDispute(disputeId: string, staffId: string, resolution: {
    resolution: 'refund_guest' | 'pay_host' | 'split' | 'dismiss';
  ...
  async getFraudReviewQueue(page: number, limit: number) {
    const from = (page - 1) * limit;
  ...
  async approveReview(reviewId: string, staffId: string) {
    const { error } = await supabaseAdmin
      .from('unified_reviews')
      .update({
        status: 'published',
  ...
  async rejectReview(reviewId: string, staffId: string, reason: string) {
    const { error } = await supabaseAdmin
      .from('unified_reviews')
      .update({
        status: 'rejected',
  ...
  async getReportedMessages(page: number, limit: number) {
    const from = (page - 1) * limit;
  ...
  async resolveMessageReport(reportId: string, staffId: string) {
    const { error } = await supabaseAdmin
      .from('message_reports')
  ...
  async deleteMessage(messageId: string, staffId: string) {
    const { error } = await supabaseAdmin
      .from('messages')
  ...
  async getPropertiesPendingReview(page: number, limit: number) {
    const from = (page - 1) * limit;
    ...
    if (error) throw new Error(`Failed to fetch pending properties: ${error.message}`);
    ...
    const enhanced = (data ?? []).map(prop => ({
      ...
      has_pricing: !!(
        (prop.property_pricing as any)?.[0]?.monthly_rent ||
        (prop.property_pricing as any)?.[0]?.asking_price
      ),
    }));
  ...
  async approveProperty(propertyId: string, staffId: string) {
    const { error } = await supabaseAdmin
      .from('properties')
      .update({
        status: 'available',
    ...
    if (error) throw new Error(`Failed to approve property: ${error.message}`);
    logger.info({ propertyId, staffId }, 'staff.property.approved');
    return { success: true };
  }

  async rejectProperty(propertyId: string, staffId: string, reason: string) {
    ...
    if (error) throw new Error(`Failed to reject property: ${error.message}`);
    logger.info({ propertyId, staffId, reason }, 'staff.property.rejected');
    return { success: true };
  }

  async flagProperty(propertyId: string, staffId: string, reason: string) {
    const { error } = await supabaseAdmin
      .from('properties')
      .update({
        status: 'off_market',
    ...
    if (error) throw new Error(`Failed to flag property: ${error.message}`);
  ...
  async getUsers(page: number, limit: number, filters: {
    ...
    if (filters.status) q = q.eq('account_status', filters.status);
    if (filters.search) {
  ...
  async suspendUser(userId: string, staffId: string, reason: string, days?: number) {
  ...
  async unsuspendUser(userId: string, staffId: string) {
    const { error } = await supabaseAdmin
      .from('users')
      .update({
  ...
  async getProperties(page: number, limit: number, filters: {
    status?: string;
    category?: string;
    search?: string;
  }) {
    ...
    if (filters.category) q = q.eq('listing_category', filters.category);
  ...
  async getShortStayBookings(page: number, limit: number, filters: {
    status?: string;
    fromDate?: string;
    toDate?: string;
  }) {
  ...
  async getLongTermBookings(page: number, limit: number, filters: { status?: string }) {
    const from = (page - 1) * limit;
    ...
    const { data, count, error } = await q;
    if (error) throw new Error(`Failed to fetch long-term bookings: ${error.message}`);
    return { bookings: data ?? [], total: count ?? 0, page, limit, pages: Math.ceil((count ?? 0) / limit) };
  }

  async getReviews(page: number, limit: number, filters: {
    propertyId?: string;
    userId?: string;
    status?: string;
  }) {
    ...
    if (filters.userId) q = q.eq('reviewer_id', filters.userId);
    if (filters.status) q = q.eq('status', filters.status);
  ...
  async getAuditLog(page: number, limit: number, filters: {
    eventType?: string;
    userId?: string;
    fromDate?: string;
    toDate?: string;
