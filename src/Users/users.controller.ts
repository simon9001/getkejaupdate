...
export class UsersController {
  ...
  // PUT /users/me/seeker
  // -------------------------------------------------------------------------
  async updateSeekerProfile(c: Context) {
    try {
      const user = c.get('user');
      const body = await c.req.json();
  ...
  // -------------------------------------------------------------------------
  async updateLandlordProfile(c: Context) {
    try {
      const user = c.get('user');
      const body = await c.req.json();
  ...
  // PUT /users/me/agent
  // -------------------------------------------------------------------------
  async updateAgentProfile(c: Context) {
    try {
      const user = c.get('user');
      const body = await c.req.json();
  ...
  // -------------------------------------------------------------------------
  async updateCaretakerProfile(c: Context) {
    try {
      const user = c.get('user');
      const body = await c.req.json();
  ...
  // -------------------------------------------------------------------------
  async updateDeveloperProfile(c: Context) {
    try {
      const user = c.get('user');
      const body = await c.req.json();
  ...
  async submitVerification(c: Context) {
    try {
      ...
      return c.json(
        { message: error.message || 'Failed to submit verification', code: isPending ? 'ALREADY_PENDING' : 'SERVER_ERROR' },
        isPending ? 409 : 500,
      );
    }
  }
  ...
  async updateUserStatus(c: Context) {
    try {
      const id = c.req.param('id');
      const { status } = await c.req.json() as { status: AccountStatus };
      ...
      return c.json(
        { message: error.message || 'Failed to update status', code: isValidation ? 'INVALID_STATUS' : 'SERVER_ERROR' },
  ...
  // -------------------------------------------------------------------------
  async updateUserRoles(c: Context) {
    try {
      const id = c.req.param('id');
      const body = await c.req.json() as { assign?: UserRole[]; revoke?: UserRole[] };
