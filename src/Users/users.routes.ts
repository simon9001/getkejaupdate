...
// ---------------------------------------------------------------------------
const authorizeAdmin: MiddlewareHandler = async (c, next) => {
  const user = c.get('user');
  const roles = (user?.roles ?? []) as string[];

  if (!roles.includes('super_admin') && !roles.includes('staff')) {
    return c.json(
      { message: 'Forbidden: staff or super_admin role required', code: 'FORBIDDEN' },
      403,
    );
  }
  await next();
...
usersRouter.put(
  '/me',
  zValidator('json', updateBaseProfileSchema),
  (c) => usersController.updateMyProfile(c),
);

usersRouter.put(
  '/me/seeker',
  zValidator('json', updateSeekerProfileSchema),
  (c) => usersController.updateSeekerProfile(c),
);

usersRouter.put(
  '/me/landlord',
  zValidator('json', updateLandlordProfileSchema),
  (c) => usersController.updateLandlordProfile(c),
);

usersRouter.put(
  '/me/agent',
  zValidator('json', updateAgentProfileSchema),
  (c) => usersController.updateAgentProfile(c),
);

usersRouter.put(
  '/me/caretaker',
  zValidator('json', updateCaretakerProfileSchema),
  (c) => usersController.updateCaretakerProfile(c),
);

usersRouter.put(
  '/me/developer',
  zValidator('json', updateDeveloperProfileSchema),
  (c) => usersController.updateDeveloperProfile(c),
);
...
usersRouter.post(
  ...
  (c) => usersController.submitVerification(c),
);
...
usersRouter.get(
  '/',
  authorizeAdmin,
  (c) => usersController.getAllUsers(c),
);

usersRouter.get(
  '/:id',
  authorizeAdmin,
  (c) => usersController.getUserById(c),
);

usersRouter.patch(
  '/:id/status',
  authorizeAdmin,
  zValidator('json', adminUpdateStatusSchema),
  (c) => usersController.updateUserStatus(c),
);

usersRouter.patch(
  '/:id/roles',
  authorizeAdmin,
  zValidator('json', adminUpdateRolesSchema),
  (c) => usersController.updateUserRoles(c),
);
